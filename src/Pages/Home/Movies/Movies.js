                         ...
                         shows.map(show => <div key={show.show.id}>
                              <div onClick={()=>navigateDetails(show.show.id)} className='relative cursor-pointer'>
                                   ...
                                   <div className='absolute bottom-0 top-0 w-full bg-gradient-to-b from-transparent to-slate-900 flex justify-between items-end px-2 py-5'>
                                        <p className=' font-semibold text-2xl font-mono z-10 text-white '>{show?.show?.name}</p>
                                        <div className='flex gap-1'><StarIcon className='w-4 text-yellow-500'></StarIcon><span className='text-yellow-200'>{show?.show?.rating?.average ? show?.show?.rating?.average : 'counting'}</span></div>
                                   </div>
                              </div>
                         </div>)
                    }
